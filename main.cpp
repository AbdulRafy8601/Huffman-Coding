...
int main()
{
	...
	while (choice)
	{
		...
		if (choice == 1)
		{
			...
			int *fb = f.getFrequency();//use association instead of coyping the frequency a new variable, as in association we can use the function of object from other class
			myQueue obj(256);//why we need to constant 256 as parameter and should replace the local parameter where it is used in called function by 256
			for (int i = 0; i < 256; i++)
			{
				if (fb[i] > 0)
				{
		...
		{
			break;
			cout << "Invalid choice. Please enter from (1,2,3)." << endl;
		}
		system("pause");
		system("cls");
	}
